...
public class OpenTracingWrapper {
  ...
  private static final Logger LOG = Logger.getLogger(OpenTracingWrapper.class.getName());
  ...
  private ContextContainer prepareSpan(String operationName) {
    ...
    Tracer.SpanBuilder spanBuilder =
        GlobalTracer.get().buildSpan(operationName).withTag(Tags.COMPONENT.getKey(), component);
    ...
    if (newGrpcContextForSpan) {
      currentContext =
          Context.current()
              .withValues(
                  OpenTracingContextKey.getKey(),
                  span,
                  OpenTracingParentContextKey.getKey(),
                  parentSpan);
  ...
  private void finishSpan(ContextContainer context) {
    ...
    try {
      ...
      LOG.info("Could not finish span");
    }
  }
  ...
  public <T, U, R> R map(
      String operationName, BiFunction<? super T, ? super U, ? extends R> func, T par1, U par2) {
    ContextContainer context = prepareSpan(operationName);
    try {
  ...
  public <T, U> void run(
      String operationName, BiConsumer<? super T, ? super U> func, T par1, U par2) {
    ContextContainer context = prepareSpan(operationName);
    try {
